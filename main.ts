 ...
 */
function getModuleKeys(obj: any): string[] {
...
async function handleQuery(queryString: string): Promise<FlowResult[]> {
	let lang = 'en';
	let count = 1;
	let category = '';
	let module = '';
	...
	for (let part of rawParts) {
		...
		if (part.includes(':')) {
			...
			if (key === 'lang') {
				...
				if (!isNaN(Number(val))) args[key] = Number(val);
	...
	const faker = (allFakers as any)[lang] || allFakers['en'];
	...
	if (queryTokens.length === 0) {
		// List all categories
		const categories = Object.keys(faker).filter(k =>
			typeof faker[k] === 'object' &&
			!k.startsWith('_') &&
		...
		if (exactMatch) {
			...
			modules.forEach(mod => {
				...
				if (meta) {
					...
					if (meta.params && meta.params.length > 0) subtitle += `\nParams: ${meta.params.join(', ')}`;
				}
				results.push({
					Title: mod,
					Subtitle: subtitle,
	...
	if (category && module) {
		try {
			...
			if (typeof func === 'function') {
				...
				const positionalOverrides: Record<string, string[]> = {
					'fromRegExp': ['pattern'],
					'slugify': ['string'],
...
function handleContextMenu(contextData: any[]): FlowResult[] {
